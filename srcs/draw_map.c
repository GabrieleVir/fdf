...
/*   Updated: 2019/03/20 06:19:32 by gvirga           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		prepare_data_bres(t_bres *data, int x_or_y, t_data **mai)
{
	if (x_or_y == 1)
	{
		(*data).x1 = ((*mai)->i % (*mai)->nb_column) * (*mai)->dst_x;
		(*data).x2 = (((*mai)->i + 1) % (*mai)->nb_column) * (*mai)->dst_x;
		(*data).y1 = ((*mai)->i / (*mai)->nb_row) * (*mai)->dst_y;
		(*data).y2 = (((*mai)->i + 1) / (*mai)->nb_row) * (*mai)->dst_y;
	}
	else
	{
		(*data).x1 = ((*mai)->i % (*mai)->nb_column) * (*mai)->dst_x;
		(*data).x2 = (((*mai)->i + (*mai)->nb_column) %
				(*mai)->nb_column) * (*mai)->dst_x;
		(*data).y1 = ((*mai)->i / (*mai)->nb_row) * (*mai)->dst_y;
		(*data).y2 = (((*mai)->i + (*mai)->nb_column) /
				(*mai)->nb_row) * (*mai)->dst_y;
	}
}
...
int				draw_map(t_data **mai, t_mlx **m_i)
{
	...
	while ((*mai)->i < (*mai)->nb_of_elems - 1)
	{
		...
		if (((*mai)->i + 1) / (*mai)->nb_row != (*mai)->nb_row)
		{
